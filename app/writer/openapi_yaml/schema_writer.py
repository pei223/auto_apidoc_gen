...
def convert_openapi_schema(
    ...
    if isinstance(param_info, ObjectSchemaParam) or isinstance(
        param_info, FirstObjectSchemaParam
    ):
        ...
        return (
            row
            if isinstance(prev_param_info, ArraySchemaParam) or prev_param_info is None
            else {param_info.name: row}
        )
    if isinstance(param_info, ArraySchemaParam):
        return {
            param_info.name: OrderedDict(
                {
                    "type": "array",
                    "items": convert_openapi_schema(param_info.items, param_info),
                }
            )
        }
    ...
    if isinstance(param_info, RefSchemaParam):
        ...
        return (
            row
            if isinstance(prev_param_info, ArraySchemaParam) or not prev_param_info
            else {param_info.name: row}
        )
    ...
    if isinstance(param_info, ValueSchemaParam):
        ...
        return (
            row
            if isinstance(prev_param_info, ArraySchemaParam)
            else {param_info.name: row}
        )
