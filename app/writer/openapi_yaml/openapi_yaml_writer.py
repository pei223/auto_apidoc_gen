...
class OpenAPIYamlFormatWriter:
    def __init__(self, entity: Entity, setting: Setting):
        ...
        self._wrap_up_api_sentences = self._entity.generate_wrap_up_sentences()
    ...
    def _parse_each_endpoint(self):
        for order in range(len(self._entity.api_info_ls)):
            ...
            wrap_up_api_sentence = self._wrap_up_api_sentences[order]
            ...
            self._tree["paths"][endpoint_url].update(
                {
                    ...
                    api_info.api_kind.method_type(
                        self._setting.is_rest
                    ).value.lower(): {
                        **self._get_method_data(api_info, wrap_up_api_sentence),
                        **{
                            "description": api_info.api_sentence,
                            "operationId": f"{api_info.api_kind.operation_word_en()}-{self._entity.entity_en_name}",
                        },
                        **{
                            "parameters": self._get_query_parameters(api_info.api_kind)
                            + self._get_header_parameters()
                        },
                    },
                }
            )
    ...
    def _get_path_parameters(self, api_kind: ApiKind) -> List[Dict[str, any]]:
        return list(
            map(
                lambda path_param: self._gen_param_dict(path_param, "path"),
                api_kind.path_parameters(),
            )
        )
    ...
    def _get_query_parameters(self, api_kind: ApiKind) -> List[Dict[str, any]]:
        return list(
            map(
                lambda path_param: self._gen_param_dict(path_param, "query"),
                api_kind.query_parameters(),
            )
        )

    def _get_header_parameters(self) -> List[Dict[str, any]]:
        return [
            convert_openapi_schema(self._setting.authorization_ref_schema()),
        ]

    def _get_method_data(
        self, api_info: ApiInfo, wrap_up_api_sentence: str
    ) -> Dict[str, any]:
        method_data = {
            "summary": wrap_up_api_sentence,
            "tags": [],
        ...
        for http_status in http_status_list:
            response_schema = (
                convert_openapi_schema(
                    api_kind.response_schema(entity_en_name=self._entity.entity_en_name)
                )
            ...
            responses[str(http_status.status_code())] = {
                "description": http_status.description(
                    self._entity.entity_ja_name, api_kind.operation_word()
                ),
                "content": {
                    "application/json": {
