...
class EndpointInfo:
    ...
    def generate_endpoint_urls(self, is_REST: bool) -> List[str]:
        return list(map(
            lambda api_kind:
            f"/{self.entity.endpoint_text}/{api_kind.rest_endpoint_extension() if is_REST else api_kind.endpoint_extension()} "
            , self.api_kind_ls))

    def to_string(self, is_REST=True):
        result_rows = [self.entity.entity_name + "API list", ]
...
def aggregate_by_entity(api_nl_names: List[str], entities: List[Entity], api_kind_ls: List[ApiKind]) -> List[
    EndpointInfo]:
    """
    ...
    endpoint_info_ls = []
    for key in d.keys():
        endpoint_info = EndpointInfo(entity=d[key]["entity"], api_nl_names=d[key]["api_nl_names"],
